locals {
  ...
  port         = 8080
  vpc = {
    id              = data.terraform_remote_state.cajasocial.outputs.vpc.vpc_id,
    private_subnets = data.terraform_remote_state.cajasocial.outputs.vpc.private_subnets
  }
  ecs                = data.terraform_remote_state.cajasocial.outputs.ecs
  https_listener_arn = data.terraform_remote_state.cajasocial.outputs.alb.https_listener_arns[0]
  security_groups    = data.terraform_remote_state.cajasocial.outputs.sg
}
